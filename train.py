...
from method.data_provider import Dataset4Training,collate_train,read_video_ids
from method.data_provider import VisDataSet4Test, TxtDataSet4Test
...
from method.losses import SetCriterion
...
def train_epoch(model, train_loader, optimizer, opt, epoch_i, criterion, training=True):
    ...
    if opt.hard_negative_start_epoch != -1 and epoch_i >= opt.hard_negative_start_epoch:
        criterion.set_hard_negative(True, opt.hard_pool_size)
    ...
    loss_meters = OrderedDict(loss_overall=AverageMeter(),glance_inter_nce_loss=AverageMeter(), glance_inter_trip_loss=AverageMeter(),
                              gaze_inter_nce_loss=AverageMeter(), gaze_inter_trip_loss=AverageMeter(),
                              gaze_intra_left_trip_loss=AverageMeter(), gaze_intra_right_trip_loss=AverageMeter(), gaze_intra_whole_trip_loss=AverageMeter(),
                              saliency_loss=AverageMeter(),
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader), desc="Training Iteration", total=num_training_examples):
        ...
        timer_start = time.time()
        # pred_res= model(batch['clip_video_features'],batch['frame_video_features'],batch['videos_mask'],
        #                         batch['text_feat'],batch['text_mask'], epoch=epoch_i)
        pred_res= model(batch, epoch=epoch_i)
        loss, loss_dict = criterion(pred_res, epoch_i)
...
def train(model, train_dataset, val_video_dataset, val_text_dataset, criterion, opt):
    # Prepare optimizer
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        if epoch_i > -1:
            train_epoch(model, train_loader, optimizer, opt, epoch_i, criterion, training=True)
...
def start_training(opt):
    ...
    loss_names = ['glance_inter_loss', 'gaze_inter_loss', 'gaze_intra_loss', 'saliency_loss', 'boundary_loss']
    logger.info(loss_names)
    criterion = SetCriterion(model_config, loss_names)
    criterion.logger = logger 
    logger.info("Start Training...")
    train(model, train_dataset, val_video_dataset, val_text_dataset, criterion, opt)
