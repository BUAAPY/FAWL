...
class frame_nce(nn.Module): 
    ...
    def forward(self, q2ctx_scores=None, contexts=None, queries=None):
        ...
        x = x.view(bsz, bsz, -1)
        nominator = x * torch.eye(x.shape[0], dtype=torch.float32, device=device)[:, :, None] # [bsz, bsz, 1]
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, query_states, key_states, value_states, attention_mask=None): 
        ...
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,) # [N, Lq, D]
...
class GlobalConv1D(nn.Module):
    def __init__(self, dim, ksize = 15, stride = 1, out_channel = 1):
        ...
        self.conv_layer = nn.Conv1d(1, out_channel, ksize, stride)
        # self.conv_layer = nn.Conv1d(1, out_channel, ksize, stride, padding='same') # padding='same' only support for stride=1
        self.out_proj = nn.Linear(dim*out_channel, dim)
        # self.Embedding = VideoEmbedding(idim, odim, sample_num=sample_num)
    ...
    def forward(self, feats, video_mask):
        ...
        feats = feats.transpose(2,1).reshape(B*dim, lv).unsqueeze(1) # [B*dim, 1, lv]
        conv_feat = self.conv_layer(feats)
        d1, channel, lo = conv_feat.shape
        out_feat = conv_feat.reshape(B, dim, channel, lo).transpose(3,1).reshape(B, lo, channel*dim) # [B, lo, c*dim]
