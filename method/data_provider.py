...
def collate_train(data):
    ...
    clip_video_features, frame_video_features, cap_feat, idxs, cap_ids, video_ids, caps= zip(*data)
    #videos
    ...
    return dict(clip_video_feat=clip_videos,
                frame_video_feat=frame_videos,
                frame_video_mask=videos_mask,
                query_feat=padded_roberta_feat,
                query_mask=roberta_mask,
                raw_captions=caps,
                video_ids=video_ids,
                cap_ids=cap_ids
                )
...
def collate_frame_val(data):
    ...
    return dict(
        clip_video_feat=clip_videos,
        frame_video_feat=frame_videos,
        frame_mask=videos_mask,
        idxs=idxs,
        video_metas=video_ids
    )
...
class Dataset4Training(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # text
        cap_feat = self.text_feat[str(cap_id)][...] # changed
        cap_tensor = torch.from_numpy(l2_normalize_np_array(cap_feat))[:self.max_desc_len]

        cap = self.captions[cap_id]

        return clip_video_feature, frame_video_feature, cap_tensor, index, cap_id, video_id, cap
        

    def __len__(self):
        return self.length

class VisDataSet4Test(data.Dataset):
...
class TxtDataSet4Test(data.Dataset):
    """
    Load captions
    """
