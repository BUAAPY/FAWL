...
import os
import pickle
from method.model import FAWL
from torch.utils.data import DataLoader
from method.data_provider import VisDataSet4Test,\
...
def get_gt(video_metas, query_metas, cap2vid):  
    v2t_gt = [] # cap2vid:  {desc_name；vid_name}
...
def compute_context_info(model, eval_dataset, opt): 
    ...
    for idx, batch in tqdm(enumerate(context_dataloader), desc="Computing context info",
        ...
        metas.extend(batch["video_metas"])
        # import pdb; pdb.set_trace() 
        clip_video_feat_ = batch["clip_video_feat"].to(opt.device, non_blocking=opt.pin_memory)
        frame_video_feat_ = batch["frame_video_feat"].to(opt.device, non_blocking=opt.pin_memory)
        frame_mask_ = batch["frame_mask"].to(opt.device, non_blocking=opt.pin_memory)

        # _frame_feat, _clip_proposal_feat, _encoded_clip_feat, _frame_props_feat  = model.encode_context(frame_video_feat_, frame_mask_)
        visual_features  = model.encode_context(frame_video_feat_, frame_mask_, clip_video_feat_)

        # _video_proposal_feat = _video_proposal_feat.cpu().numpy() # ori
        clip_feat.append(visual_features["glance_visual_feature"]) # _frame_feat
        frame_feat.append(visual_features["gaze_visual_feature"])
        frame_mask.append(frame_mask_) # clip_proposal_feat: 
        if clip_proposal_feat is None:
            clip_proposal_feat = torch.zeros((n_total_vid, visual_features["glance_props_feature"].shape[1], opt.hidden_size)).type_as(visual_features["glance_props_feature"])
            clip_proposal_feat[idx * bsz:(idx + 1) * bsz] = visual_features["glance_props_feature"]
        else:
            clip_proposal_feat[idx * bsz:(idx + 1) * bsz] = visual_features["glance_props_feature"]
        # frame props feat
        if  frame_props_feat is None:
            frame_props_feat = torch.zeros((n_total_vid, visual_features["gaze_props_feature"].shape[1], opt.hidden_size)).type_as(visual_features["gaze_props_feature"])
            frame_props_feat[idx * bsz:(idx + 1) * bsz] = visual_features["gaze_props_feature"]
        else:
            frame_props_feat[idx * bsz:(idx + 1) * bsz] = visual_features["gaze_props_feature"]
...
def compute_query2ctx_info(model, eval_text_dataset, eval_video_dataset, opt, ctx_info):
    ...
    for idx, batch in tqdm(enumerate(query_eval_loader), desc="Computing query2video scores", total=len(query_eval_loader)):
        ...
        # print("***", idx, "***")
        _query_metas = batch["query_metas"]
        query_metas.extend(_query_metas) # changed
        query_feat = batch["query_feat"].to(opt.device) # , non_blocking=opt.pin_memory
        query_mask = batch["query_mask"].to(opt.device) # , non_blocking=opt.pin_memory
        num_props_for_each_video = opt.npev # 10

        infer_res = model.inference(
            query_feat, query_mask, ctx_info,  num_props_for_each_video
        )

        _score_sum = opt.clip_scale_w*infer_res["glance_vr_scores"] + opt.frame_scale_w*infer_res["gaze_vr_scores"]

        score_sum.append(_score_sum)

        num_query, num_video, num_props, _ = infer_res["topk_props"].shape 
        _topk_props = infer_res["topk_props"].reshape(num_query, num_video*num_props, 2) 
        _topk_props_left = _topk_props[:,:,0]
        _topk_props_right = _topk_props[:,:,1]
        _topk_prop_scores = infer_res["topk_prop_scores"].reshape(num_query, num_video*num_props)

        ## select global props
        max_vcmr_props = min(opt.max_vcmr_props, num_video*num_props) # 1000
        _global_topk_prop_scores, _global_topk_prop_indices = torch.topk(_topk_prop_scores, max_vcmr_props, dim=1)


        _selected_props_left_sorted = torch.gather(_topk_props_left, 1, _global_topk_prop_indices)
        _selected_props_right_sorted = torch.gather(_topk_props_right, 1, _global_topk_prop_indices)
        _selected_props_sorted = torch.stack([_selected_props_left_sorted, _selected_props_right_sorted], dim=-1).cpu().numpy().copy() # [num_query, max_vcmr_props, 2]
        _global_topk_prop_scores = _global_topk_prop_scores.cpu().numpy().copy()
        _global_topk_prop_indices = _global_topk_prop_indices.cpu().numpy().copy()

        # process queries one by one
        for i, single_query_id in enumerate(_query_metas): 
            single_query_sorted_score = _global_topk_prop_scores[i] # 
            ...
            for j, single_prop in enumerate(_selected_props_sorted[i]): 
                vid_idx_local, prop_idx = np.unravel_index(_global_topk_prop_indices[i, j], shape=(num_video, num_props)) 
                vid_name = ctx_info["video_metas"][vid_idx_local] 
    ...
    return  sum_sorted_indices, query_metas, vcmr_res_dict
...
def eval_epoch(model, val_video_dataset, val_text_dataset, opt, res_dir, save_submission_filename, epoch=999):
    ...
    sum_sorted_indices, query_metas, vcmr_res_dict = compute_query2ctx_info(model, val_text_dataset, val_video_dataset, opt, context_info)
    video_metas = context_info['video_metas']


    v2t_gt, t2v_gt = get_gt(video_metas, query_metas, val_text_dataset.cap2vid) 
    ...
    # nms or not  
    metrics = eval_retrieval(vcmr_res_dict_nmsed, val_text_dataset.cap_data, 
                            iou_thds=IOU_THDS, match_number=not opt.debug, verbose=opt.debug, 
                            use_desc_type=opt.collection=='tvr') 
    logger.info('-'*40)
